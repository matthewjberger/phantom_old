...
impl<'a, R, T, E> StateMachine<'a, R, T, E> {
    pub fn new(
        initial_state: impl State<R, T, E> + 'a,
    ) -> StateMachine<'a, R, T, E> {
        Self { running: false, states: vec![Box::new(initial_state)] }
    }
    ...
    pub fn handle_event(&mut self, data: StateData<'_, R, T>, event: E) {
        ...
        if self.running {
            let transition = match self.states.last_mut() {
                Some(state) => {
                    state.handle_event(StateData { resources, data }, event)
                }
    ...
    pub fn transition(
        &mut self,
        request: Transition<R, T, E>,
        data: StateData<'_, R, T>,
    ) {
        if self.running {
            match request {
                Transition::None => (),
...
mod tests {
    ...
    fn simulate_game() -> Result<()> {
        ...
        let intro_state = IntroState { countdown: intro_countdown };
